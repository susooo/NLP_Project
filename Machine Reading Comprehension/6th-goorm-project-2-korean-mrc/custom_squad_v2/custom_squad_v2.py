...
class SquadV2(datasets.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        SquadV2Config(name="custom-squad_v2", version=datasets.Version("0.0.1"), description="SQuAD plaint text version 2"),
    ]
    ...
    def _split_generators(self, dl_manager):
        ...
        if IN_COLAB:
            return [
                datasets.SplitGenerator(name=datasets.Split.TRAIN, gen_kwargs={"filepath": '/content/custom_squad_v2/train.json'}),
                #datasets.SplitGenerator(name=datasets.Split.TEST, gen_kwargs={"filepath": 'test.json'}),
            ]
        else:
            return [
                datasets.SplitGenerator(name=datasets.Split.TRAIN, gen_kwargs={"filepath": curpath+'/custom_squad_v2/train.json'}),
                #datasets.SplitGenerator(name=datasets.Split.TEST, gen_kwargs={"filepath": 'test.json'}),
            ]
