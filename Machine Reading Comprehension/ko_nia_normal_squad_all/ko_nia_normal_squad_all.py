...
class SquadV2(datasets.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        SquadV2Config(name="ko_nia_normal_squad_all", version=datasets.Version("0.0.1"), description="SQuAD plaint text version 2"),
    ]
    ...
    def _split_generators(self, dl_manager):
        ...
        if IN_COLAB:
            return [
                datasets.SplitGenerator(name=datasets.Split.TRAIN, gen_kwargs={"filepath": '/content/ko_nia_normal_squad_all/ko_nia_normal_squad_all.json'}),
                #datasets.SplitGenerator(name=datasets.Split.TEST, gen_kwargs={"filepath": 'test.json'}),
            ]
        else:
            return [
                datasets.SplitGenerator(name=datasets.Split.TRAIN, gen_kwargs={"filepath": curpath+'/ko_nia_normal_squad_all/ko_nia_normal_squad_all.json'}),
                #datasets.SplitGenerator(name=datasets.Split.TEST, gen_kwargs={"filepath": 'test.json'}),
            ]
